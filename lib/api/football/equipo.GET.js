    ...
    server.route({
        ...
        config:{
            ...
            handler: (request, reply) => {
              var db = server.plugins['hapi-mongodb'].db[0];
              db.collection('partido').find({}).toArray((err, result) =>{//toAarray si no tenemos un objeto cursor que no puede devolver reply
        if (err) {
